...
public:
    explicit RequestQueue(const SearchServer& search_server)
        : search_server_(search_server), current_time_(0), no_result_count_(0) {
    }
    ...
    // Обёртка для метода поиска без предиката
    ...
    // Возвращает количество запросов с нулевым результатом за последние сутки
...
private:
    ...
    struct QueryResult {
        int result_count; // Количество найденных документов
        int time;         // Время запроса
    };

    // Добавляет запрос в очередь и обновляет статистику
    void AddRequest(int result_count) {
        // Увеличиваем время на одну минуту
        ++current_time_;

        // Удаляем устаревшие запросы (старше 1440 минут)
        ...
        requests_.push_back({result_count, current_time_});

        // Обновляем количество запросов с нулевым результатом
        if (result_count == 0) {
            ++no_result_count_;
        }
    }

    const SearchServer& search_server_; // Ссылка на поисковый сервер
    deque<QueryResult> requests_;       // Очередь запросов
    int current_time_;                  // Текущее время (в минутах)
    int no_result_count_;               // Количество запросов с нулевым результатом
    const static int min_in_day_ = 1440; // Количество минут в сутках
};